...
describe('структуровані дані вакансій', якщоЗібрано, () => {
  ...
  test('кожна вакансія має власну розмітку JobPosting', () => {
    const вакансії = блоки().filter((b) => b?.['@type'] === 'JobPosting')
    assert.equal(вакансії.length, 5)
  })
  ...
  test('вигадана вилка не потрапляє в Google Jobs', () => {
    ...
    if (підтверджено) assert.equal(зСумою, 5, 'цифри підтверджені, а в розмітці їх немає')
    else assert.equal(зСумою, 0, 'непідтверджена вилка поїхала в структуровані дані')
  })
})
