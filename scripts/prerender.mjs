 ...
 * GitHub Pages віддає такий файл напряму, а React далі перемальовує сторінку як завжди.
 *
 * Якщо браузер не піднявся — білд не валимо: лишається звичайний SPA.
 */
...
try {
  ...
  server.close()
  console.warn(`prerender: браузер не запустився (${err.message}) — лишаю звичайний SPA`)
  process.exit(0)
}
