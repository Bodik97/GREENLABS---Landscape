...
### Заміна картинок у `public/`

`vercel.json` віддає `/img` і `/logo` з кешем на місяць. Імена там не хешовані,
тож **підміна вмісту під тим самим іменем не дійде до тих, хто вже був на
сайті** — до місяця вони бачитимуть старий файл, і `stale-while-revalidate` тут
не рятує, бо вмикається вже після того, як строк вийшов.

Тому нову картинку треба класти під **новим іменем** і правити посилання. Файли
з розміром у назві (`hero-1280.webp`, `logo-v2-208.webp`) це роблять самі собою;
пильнувати треба ті, що зі сталим іменем — `og.jpg`, `banner-*.webp`,
`logo-32.webp`, знімки послуг. `og.jpg` одного разу вже підмінили на місці
(коміт `f494251`).

Це не стосується фото з Sanity: там адреси містять хеш вмісту.
