...
export function BeforeAfterSlider({ img, label }: { img: string; label: string }) {
  const [pos, setPos] = useState(50)
  return (
    <div className="relative rounded-2xl overflow-hidden aspect-4/3 select-none bg-green">
      <img src={img} alt={`${label} — після`} className="absolute inset-0 w-full h-full object-cover pointer-events-none" loading="lazy" />
      <div className="absolute inset-0 overflow-hidden pointer-events-none" style={{ clipPath: `inset(0 ${100 - pos}% 0 0)` }}>
        <img src={img} alt={`${label} — до`} className="absolute inset-0 w-full h-full object-cover grayscale brightness-75 saturate-50" loading="lazy" />
      </div>
      ...
      <span className="absolute top-3 left-3 bg-black/55 text-white text-[10px] font-display font-semibold uppercase tracking-wider px-2.5 py-1 rounded-full pointer-events-none">До</span>
      <span className="absolute top-3 right-3 bg-black/55 text-white text-[10px] font-display font-semibold uppercase tracking-wider px-2.5 py-1 rounded-full pointer-events-none">Після</span>
