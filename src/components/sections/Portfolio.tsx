...
export function Portfolio({ bg = 'bg-cream' }: { bg?: string }) {
  ...
  if (loading) return <section id="portfolio" className={`py-24 min-h-150 ${bg}`} aria-hidden="true" />

  return <WorkCards id="portfolio" eyebrow="Портфоліо" title="Наші роботи" items={works ?? []} bg={bg} />
}
